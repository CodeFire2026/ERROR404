#include <iostream>
#include <ctime>
#include <cstdlib>
using namespace std;

int main(){
	
	//VARIABLES
	int lim_inf = 1;
	int lim_sup = 20;
	int tam_array = 10;
	int lista[tam_array] = {0};
	int aleatorio = 0;
	bool interruptor = true;
	int aux = 0;
	int intercambios = 0;
	int pasadas = 0;
	bool repetido = false;
	
	srand(time(NULL)); //INICIALIZAR SEMILLA
	
	//FOR QUE LLENA EL VECTOR CON NUMEROS ALEATORIOS
	cout<<endl; //SALTO DE LINEA
	for(int i=0; i<tam_array; i++){ //FOR QUE RECORRE LISTA
		repetido = false;
		aleatorio = lim_inf + rand() % (lim_sup-1-lim_inf);
		// FOR QUE RECORRE EL ARRAY EN BUSCA DE REPETIDO
		for(int j=0; j<tam_array; j++){
			if(lista[j] == aleatorio){
				repetido = true;
			}
		}
		
		if(repetido == true){
			i--;
		}else{
			lista[i] = aleatorio;
			cout<<"["<<lista[i]<<"]";
		}
	}
	
	//METODO DE BURBUJA
	// FOR QUE INDICA LA CANTIDAD DE PASADAS
	for(int i=0; (i<tam_array-1 && interruptor == true); i++){
		pasadas++;
		interruptor = false;
		//FOR QUE INDICA LA CANTIDAD DE COMPARACIONES
		for(int j=0; (j<tam_array-1-i); j++){
			if(lista[j] > lista[j+1]){
				// INTERCAMBIO
				aux = lista[j];
				lista[j] = lista[j+1];
				lista[j+1] = aux;
				interruptor = true;
				intercambios++;
			}
		}
	}
	// IMPRIME EL ARRAY
	cout<<endl<<endl; //SALTO DE LINEA
	for(int i=0; i<tam_array; i++){
		cout<<"["<<lista[i]<<"]";
	}
	cout<<endl<<endl<<"Cantidad de pasadas: "<<pasadas<<endl;
	cout<<"Cantidad de intercambios: "<<intercambios<<endl;
	
	
	
	return(0);
}